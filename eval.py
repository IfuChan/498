...
from dataset.rsna_pneumonia import get_rsna_pneumonia_dataloaders

from helper.loops import train_distill as train, validate_vanilla, validate_distill, validate_vanilla_new
from helper.loops import validate
from helper.util import save_dict_to_json, reduce_tensor, adjust_learning_rate

# from crd.criterion import CRDLoss
# from distiller_zoo import DistillKL, HintLoss, Attention, Similarity, VIDLoss, SemCKDLoss, DKD
...
def parse_option():
    ...
    # dataset and model
    parser.add_argument('--dataset', type=str, default='rsna_pneumonia', choices=['cifar100', 'imagenet', 'rsna_pneumonia'], help='dataset')
    ...
    parser.add_argument('--distill', type=str, default='kd', choices=['kd', 'hint', 'attention', 'similarity', 'vid',
                                                                      'crd', 'semckd','srrl', 'simkd', 'dkd', 'merge'])
    ...
    parser.add_argument('--deterministic', default=True, action='store_true', help='Make results reproducible')
    parser.add_argument('--skip-validation', action='store_true', help='Skip validation of teacher')
    
    opt = parser.parse_args()

    # wandb.init(
    #     project='CSE499-CSE498R', entity='sudiptabhatta'
    # )
...
def load_teacher(model_path, n_cls, gpu=None, opt=None):
    ...
    model = model_dict['ShuffleV2'](num_classes=n_cls)
...
def main_worker( gpu, ngpus_per_node, opt):
    ...
    elif opt.dataset == 'rsna_pneumonia':
    ...
    #     # #couldnt find anything to add
    #     # print('DKD running')
    #     criterion_kd = DKD(model_s, model_t)
    # elif opt.distill == 'merge':
    #     s_n = feat_s[-2].shape[1]
    #     t_n = feat_t[-2].shape[1]
    #     model_simkd = SimKD(s_n= s_n, t_n=t_n, factor=opt.factor)
    #     criterion_kd = nn.MSELoss()
    #     module_list.append(model_simkd)
    #     trainable_list.append(model_simkd)
    #     criterion_div = DKD(model_s, model_t)
    # else:
    ...
    if opt.dataset == 'cifar100':
        ...
        train_loader, val_loader = get_rsna_pneumonia_dataloaders(batch_size=opt.batch_size, num_workers=opt.num_workers)
    else:
    ...
    teacher_acc, _, _ = validate_vanilla_new(-1, val_loader, model_t, criterion_cls, opt)
    ...
    true_values, pred_values= validate(val_loader, model_t, opt)
    
    fpr, tpr, thresholds = metrics.roc_curve(true_values, pred_values, pos_label=1)
    ...
    plt.savefig('yo2.png')
    x=roc_auc_score(true_values, pred_values)
    print('Auc Score: ')
    print(x)
    ...
    #     metrics.to_csv(opt.save_folder+"/metrics_train.csv", mode=('w' if epoch==1 else 'a'), index=False, header=(True if epoch==1 else False))
    ...
    #     if not opt.multiprocessing_distributed or opt.rank % ngpus_per_node == 0:
    #         print(' * Epoch {}, GPU {}, Acc@1 {:.3f}, Acc@5 {:.3f}'.format(epoch, opt.gpu, train_acc, train_acc_top5, time2 - time1))
    ...
    #     metrics.to_csv(opt.save_folder+"/metrics_validation.csv", mode=('w' if epoch==1 else 'a'), index=False, header=(True if epoch==1 else False))
    #     if opt.dali is not None:
    #         train_loader.reset()
    #         val_loader.reset()
