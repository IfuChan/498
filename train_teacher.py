...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='cifar100', choices=['cifar100', 'imagenet', 'rsna_pneumonia'], help='dataset')
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        test_acc, test_acc_top5, test_loss = validate_vanilla(epoch, val_loader, model, criterion, opt)
        metrics = pd.DataFrame([{'epoch': epoch,
                                 'test_acc': test_acc,
                                 'test_loss': test_loss,
                                 }])
