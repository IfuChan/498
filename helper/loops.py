...
import time
import torch
from .util import AverageMeter, accuracy, reduce_tensor, PredList, update_classification_report
...
def train_vanilla(epoch, train_loader, model, criterion, optimizer, opt):
    ...
    for idx, batch_data in enumerate(train_loader):
        ...
        if opt.dataset != 'rsna_pneumonia':
          ...
          metrics = accuracy(output, labels, topk=(1,2))
          cl_report.update(labels, output)
          top1.update(metrics[0].item(), images.size(0))
          top5.update(metrics[1].item(), images.size(0))
          batch_time.update(time.time() - end)
        # ===================backward=====================
    ...
    # return top1.avg, top5.avg, losses.avg, precision.avg, recall.avg
...
def train_distill(epoch, train_loader, module_list, criterion_list, optimizer, opt):
    ...
    for idx, data in enumerate(train_loader):
        ...
        if opt.distill != 'merge':
            # print('Not merge loss_div!!!!!!!!!!!!')
            loss_div = criterion_div(logit_s, logit_t)
        ...
        if opt.distill == 'kd':
            ...
            loss_kd=criterion_kd(images, labels, epoch)
        elif opt.distill == 'merge':
            # print('YES Merge loss_div!!!!!!!!!!!!')
            loss_div = criterion_div(images, labels, epoch) #DKD loss
            trans_feat_s, trans_feat_t, pred_feat_s = module_list[1](feat_s[-2], feat_t[-2], cls_t)
            logit_s = pred_feat_s
            loss_kd = criterion_kd(trans_feat_s, trans_feat_t) #SimKD loss
        else:
        ...
        if opt.distill != 'merge':
            ...
            loss_simkd =  loss_kd #simkd
            loss_dkd =  loss_cls +  loss_div #dkd
            alpha = 0.3
            loss = alpha * loss_simkd + (1 - alpha) * loss_dkd
        ...
        if opt.dataset != 'rsna_pneumonia':
          ...
          metrics = accuracy(logit_s, labels, topk=(1,2))
          cl_report.update(labels, logit_s)
          # print(logit_s)
          # print('test=>>>>>>',pre, rec)
          top1.update(metrics[0].item(), images.size(0))
          top5.update(metrics[1].item(), images.size(0))
...
def validate_vanilla(epoch, val_loader, model, criterion, opt):
    """validation"""
...
def validate_distill(epoch, val_loader, module_list, criterion, opt):
    ...
    with torch.no_grad():
        ...
        for idx, batch_data in enumerate(val_loader):
            ...
            # compute output
            if opt.distill == 'simkd' or opt.distill == 'merge':
...
def validate(val_loader, model, opt):
    ...
    with torch.no_grad():
        for idx, batch_data in enumerate(val_loader):
            ...
            output = model(images)
            cl_report.update(labels, output)
    pred_values=cl_report.y_pred
    true_values=cl_report.y_true

    return true_values, pred_values

def validate_vanilla_new(epoch, val_loader, model, criterion, opt):
    """validation"""
